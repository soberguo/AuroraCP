...
class Swin3DTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        shortcut = x
        x = x.view(B, C, H, W, D)#[10, 4, 32, 64, 512]
        ...
        if not all(s == 0 for s in ss):
            ...
            shifted_x = x#[10, 4, 32, 64, 512]
            attn_mask = None

        # Pad the input to multiple of window size.
        pad_size = ((-C) % ws[0], (-H) % ws[1], (-W) % ws[2])
        shifted_x = pad_3d(shifted_x, pad_size)#[10, 4, 32, 64, 512]

        # Partition the patches/tokens into windows.
        x_windows = window_partition_3d(shifted_x, ws)  # (nW*B, ws, ws, D) [720, 2, 6, 12, 512]
...
class PatchMerging3D(nn.Module):
    ...
    def forward(self, x: torch.Tensor, input_resolution: tuple[int, int, int]) -> torch.Tensor:
        ...
        """
        x = self._merge(x, input_resolution)#torch.Size([4, 8192, 512])  (4, 32, 64)
        x = self.norm(x)
        x = self.reduction(x)
        return x
...
class Swin3DTransformerBackbone(nn.Module):
    ...
    def forward(
        ...
        assert x.shape[1] == patch_res[0] * patch_res[1] * patch_res[2], _msg#[bs,8192,512]  torch.Size([bs, 2048, 256])
        ...
        lead_times = lead_hours * torch.ones(x.shape[0], dtype=torch.float32, device=x.device)#torch.Size([bs])
