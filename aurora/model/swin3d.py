...
class WindowAttention(nn.Module):
    ...
    def forward(
        ...
        x = self.proj(x) + self.lora_proj(x,hyp_x, rollout_step)
        x = self.proj_drop(x)
        return x
...
class Swin3DTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        attn_windows = self.attn(x_windows,hyp_windows, mask=attn_mask, rollout_step=rollout_step)
...
class BasicLayer3D_enc(nn.Module):
    ...
    def forward(
        ...
        for i,blk in enumerate(self.blocks):
            x = blk(x,hyp_x, c, res, rollout_step)
...
class Basic3DEncoderLayer(BasicLayer3D_enc):
    """A basic 3D Swin Transformer encoder layer. Used for FSDP, which requires a subclass."""


class Basic3DDecoderLayer(BasicLayer3D_dec):
